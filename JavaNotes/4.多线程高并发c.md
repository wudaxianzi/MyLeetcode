...
<img src="D:\算法Typora\picture\JUC-两阶段终止模式.png" style="zoom: 67%;" />
...
![](D:\算法Typora\picture\JUC-线程6种状态.png)
...
* Mark Word 结构：最后两位是**锁标志位**
  ...
  ![](D:\算法Typora\picture\JUC-Monitor-MarkWord结构32位.png)

* 64 位虚拟机 Mark Word：

  ![](D:\算法Typora\picture\JUC-Monitor-MarkWord结构64位.png)
...
![](D:\算法Typora\picture\JUC-锁升级过程.png)
...
<img src="D:\算法Typora\picture\JUC-Monitor-MarkWord结构64位.png" style="zoom: 67%;" />
...
* 创建锁记录（Lock Record）对象，每个线程的**栈帧**都会包含一个锁记录的结构，存储锁定对象的 Mark Word
  ...
  ![](D:\算法Typora\picture\JUC-轻量级锁原理1.png)

* 让锁记录中 Object reference 指向锁住的对象，并尝试用 CAS 替换 Object 的 Mark Word，将 Mark Word 的值存入锁记录

* 如果 CAS 替换成功，对象头中存储了锁记录地址和状态 00（轻量级锁） ，表示由该线程给对象加锁
  ![](D:\算法Typora\picture\JUC-轻量级锁原理2.png)
...
* 如果 CAS 失败，有两种情况：
  ...
  * 如果是线程自己执行了 synchronized 锁重入，就添加一条 Lock Record 作为重入的计数
  ...
  ![](D:\算法Typora\picture\JUC-轻量级锁原理3.png)
...
* 当 Thread-1 进行轻量级加锁时，Thread-0 已经对该对象加了轻量级锁
  ...
  ![](D:\算法Typora\picture\JUC-重量级锁原理1.png)

* Thread-1 加轻量级锁失败，进入锁膨胀流程：为 Object 对象申请 Monitor 锁，**通过 Object 对象头获取到持锁线程**，将 Monitor 的 Owner 置为 Thread-0，将 Object 的对象头指向重量级锁地址，然后自己进入 Monitor 的 EntryList BLOCKED

  ![](D:\算法Typora\picture\JUC-重量级锁原理2.png)
...
* 自旋成功的情况：
      <img src="D:\算法Typora\picture\JUC-自旋成功.png" style="zoom: 80%;" />

* 自旋失败的情况：

  <img src="D:\算法Typora\picture\JUC-自旋失败.png" style="zoom:80%;" />
...
![](D:\算法Typora\picture\JUC-park原理1.png)
...
* 先 unpark：
  ...
  ![](D:\算法Typora\picture\JUC-park原理2.png)
...
![](D:\算法Typora\picture\JUC-保护性暂停.png)
...
![](D:\算法Typora\picture\JUC-保护性暂停多任务版.png)
...
![](D:\算法Typora\picture\JUC-生产者消费者模式.png)
...
![](D:\算法Typora\picture\JMM内存模型.png)
...
<img src="D:\算法Typora\picture\JMM-内存交互.png" style="zoom: 67%;" />
...
![](D:\算法Typora\picture\JMM-可见性例子.png)
...
<img src="D:\算法Typora\picture\JMM-CPU缓存结构.png" style="zoom: 50%;" />
...
<img src="D:\算法Typora\picture\JUC-内存伪共享.png" style="zoom: 67%;" />
...
<img src="D:\算法Typora\picture\JUC-缓存一致性.png" style="zoom:80%;" />
...
* 读屏障（lfence，Load Barrier）保证在该屏障之后的，对共享变量的读取，从主存刷新变量值，加载的是主存中最新数据
  ...
  <img src="D:\算法Typora\picture\JMM-volatile保证可见性.png" style="zoom:67%;" />
...
* 有序性的保证也只是保证了本线程内相关代码不被重排序
  ...
  <img src="D:\算法Typora\picture\JMM-volatile不能保证原子性.png" style="zoom:67%;" />
...
![](D:\算法Typora\picture\JMM-DCL出现的问题.png)
...
![](D:\算法Typora\picture\JUC-ThreadLocal数据结构JDK8前.png)
...
![](D:\算法Typora\picture\JUC-ThreadLocal数据结构JDK8后.png)
...
* set()：添加数据，ThreadLocalMap 使用**线性探测法来解决哈希冲突**
  ...
  * 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
  ...
  ![](D:\算法Typora\picture\JUC-replaceStaleEntry流程.png)
...
* 探测式清理：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让**数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  <img src="D:\算法Typora\picture\JUC-ThreadLocal探测式清理1.png" style="zoom:67%;" />

  <img src="D:\算法Typora\picture\JUC-ThreadLocal探测式清理2.png" style="zoom:67%;" />
...
* 如果 key 使用强引用：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  <img src="D:\算法Typora\picture\JUC-ThreadLocal内存泄漏强引用.png" style="zoom:67%;" />
...
* 如果 key 使用弱引用：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  <img src="D:\算法Typora\picture\JUC-ThreadLocal内存泄漏弱引用.png" style="zoom:67%;" />
...
* 当一个节点入队：
  ...
  ![](D:\算法Typora\picture\JUC-LinkedBlockingQueue入队流程.png)
...
* `h = head` → `first = h.next` 
  ...
  ![](D:\算法Typora\picture\JUC-LinkedBlockingQueue出队流程1.png)

* `h.next = h` → `head = first`

  ![](D:\算法Typora\picture\JUC-LinkedBlockingQueue出队流程2.png)
...
![](D:\算法Typora\picture\JUC-线程池工作原理.png)
...
![](D:\算法Typora\picture\JUC-newSingleThreadExecutor.png)
...
* 状态表示：
  ...
  ![](D:\算法Typora\picture\JUC-线程池状态转换图.png)
...
* 请求的共享资源被占用，AQS 用 CLH 队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  <img src="D:\算法Typora\picture\JUC-AQS原理图.png" style="zoom: 80%;" />
...
* 使用了 FIFO 先入先出队列，并不支持优先级队列，**同步队列是双向链表，便于出队入队**
  ...
  ![](D:\算法Typora\picture\JUC-AQS队列设计.png)
...
<img src="D:\算法Typora\picture\JUC-ReentrantLock-非公平锁1.png" style="zoom:80%;" />
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列，前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  <img src="D:\算法Typora\picture\JUC-ReentrantLock-非公平锁2.png" style="zoom:80%;" />
...
* 再有多个线程经历竞争失败后：
  ...
  ![](D:\算法Typora\picture\JUC-ReentrantLock-非公平锁3.png)
...
* 唤醒的线程会从 park 位置开始执行，如果加锁成功（没有竞争），会设置
  ...
  * 原本的 head 因为从链表断开，而可被垃圾回收（图中有错误，原来的头节点的 waitStatus 被改为 0 了）
  ...
  ![](D:\算法Typora\picture\JUC-ReentrantLock-非公平锁4.png)
...
* 如果这时有其它线程来竞争**（非公平）**，例如这时有 Thread-4 来了并抢占了锁
  ...
  * Thread-1 再次进入 acquireQueued 流程，获取锁失败，重新进入 park 阻塞
  ...
  ![](D:\算法Typora\picture\JUC-ReentrantLock-非公平锁5.png)
...
* 开始 Thread-0 持有锁，调用 await，线程进入 ConditionObject 等待，直到被唤醒或打断，调用 await 方法的线程都是持锁状态的，所以说逻辑里**不存在并发**
  ...
  ![](D:\算法Typora\picture\JUC-ReentrantLock-条件变量1.png)
...
* fullyRelease 中会 unpark AQS 队列中的下一个节点竞争锁，假设 Thread-1 竞争成功
  ...
  ![](D:\算法Typora\picture\JUC-ReentrantLock-条件变量2.png)
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  ![](D:\算法Typora\picture\JUC-ReentrantLock-条件变量3.png)
...
* 补充情况：查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询
  ...
  <img src="D:\算法Typora\picture\JUC-ReentrantReadWriteLock缓存.png" style="zoom:80%;" />
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  <img src="D:\算法Typora\picture\JUC-ReentrantReadWriteLock加锁1.png" style="zoom: 80%;" />

* 这种状态下，假设又有 t3 r.lock，t4 w.lock，这期间 t1 仍然持有锁，就变成了下面的样子

  ![](D:\算法Typora\picture\JUC-ReentrantReadWriteLock加锁2.png)
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  <img src="D:\算法Typora\picture\JUC-ReentrantReadWriteLock解锁1.png" style="zoom: 67%;" />
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  <img src="D:\算法Typora\picture\JUC-ReentrantReadWriteLock解锁2.png" style="zoom: 67%;" />
...
<img src="D:\算法Typora\picture\JUC-CyclicBarrier工作原理.png" style="zoom: 80%;" />
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  ![](D:\算法Typora\picture\JUC-Semaphore工作流程1.png)
...
* 这时 Thread-4 释放了 permits，状态如下
  ...
  ![](D:\算法Typora\picture\JUC-Semaphore工作流程2.png)
...
![ConcurrentHashMap数据结构](D:\算法Typora\picture\ConcurrentHashMap数据结构.png)
...
* transfer()：数据转移到新表中，完成扩容
  ...
  ![](D:\算法Typora\picture\JUC-ConcurrentHashMap-LastRun机制.png)
...
* 缺点：Segments 数组默认大小为16，这个容量初始化指定后就不能改变了，并且不是懒惰初始化
  ...
  ![](D:\算法Typora\picture\JUC-ConcurrentHashMap 1.7底层结构.png)
...
<img src="D:\算法Typora\picture\JUC-CopyOnWriteArrayList弱一致性.png" style="zoom:80%;" />
...
![](D:\算法Typora\picture\JUC-ConcurrentSkipListMap数据结构.png)
...
* findPredecessor()：寻找前置节点
  ...
  ![](D:\算法Typora\picture\JUC-ConcurrentSkipListMap-Put流程.png)
...
* remove()
  ...
  ![](D:\算法Typora\picture\JUC-ConcurrentSkipListMap-remove流程.png)
...
![](D:\算法Typora\picture\JUC-ConcurrentLinkedQueue入队操作1.png)

![](D:\算法Typora\picture\JUC-ConcurrentLinkedQueue入队操作2.png)

![](D:\算法Typora\picture\JUC-ConcurrentLinkedQueue入队操作3.png)
...
![](D:\算法Typora\picture\JUC-ConcurrentLinkedQueue出队操作1.png)

![](D:\算法Typora\picture\JUC-ConcurrentLinkedQueue出队操作2.png)

![](D:\算法Typora\picture\JUC-ConcurrentLinkedQueue出队操作3.png)
