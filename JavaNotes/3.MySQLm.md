...
| 默认约束  | 保存数据时，如果未指定该字段 的值，则采用默认值 | DEFAULT  |
...
```mysql
create table user(
	id int primary key auto_increment,-- 插入数据时可以不用插入id
...
实现：**建立第三张中间表，中间表至少包含[两个外键]，分别关联两方主键**  
...
- 联合查询的多张表列数必须保持一致，字段类型也需保持一致；

### 子查询

SQL语句中**嵌套SELECT语句**，称谓嵌套查询，又称子查询。  
...
```mysql
select e.name,e.age,e.job,d.name from emp e join mydb.dept d on d.id = e.dept_id where e.age < 30;
```
...
```mysql
select e.*,d.name from emp e left join mydb.dept d on d.id = e.dept_id where (e.age > 40);
```
...
select e.Name,s.Money from(Employee e)
```
...
| 幻读  | 一个事务按照条件查询数据时，没有对应的数据行，但是再**插入数据时，又发现这行数据已经存在** |
...
- √表示在当前隔离级别下该问题会出现
- Serializable 性能最低，数据最安全，会造成阻塞；Read uncommitted 性能最高，数据安全性最差
...
📕**存储引擎**：就是存储数据、建立索引、更新/查询数据等技术的实现方式。存储引擎是**基于表**而不是基于库的，所以存储引擎也可以被称为表引擎。  
默认存储引擎是InnoDB。  
...
- MyISAM: 如果应用是以读操作和插入操作为主，只有很少的更新和删除操作，并且对事务的完整性、并发性要求不高，那这个存储引擎是非常合适的。(比如日志，电商中的访问足迹等)
- Memory: 将所有数据保存在内存中，访问速度快，通常用于临时表及**缓存**。Memory 的缺陷是对表的大小有限制，太大的表无法缓存在内存中，而且无法保障数据的安全性

电商中的足迹和评论适合使用 MyISAM 引擎，缓存适合使用 Memory 引擎。

ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password by 'wb812189';
...
root下实时查看慢查询日志：在/var/lib/mysql中`tail -f VM-4-10-ubuntu-slow.log`
...
查看慢查询日志开关状态：  
`show variables like 'slow_query_log';`  默认是关闭的
...
- Memory
- 🐧InnoDB: 具有**自适应hash功能**，hash索引是存储引擎根据 B+Tree 索引在指定条件下自动构建的
...
![image-20220721213532520](D:\算法Typora\picture\image-20220721213532520.png)
...
![image-20220721214059020](D:\算法Typora\picture\image-20220721214059020.png)
...
load data local infile '/var/lib/mysql-files/load_user_100w_sort.sql' into table tb_user fields terminated by ',' lines terminated by '\n';
LOAD DATA LOCAL INFILE '/var/lib/mysql-files/load_user_100w_sort.sql' INTO TABLE 'tb_user' FIELDS `TERMINATED` BY ',' LINES `TERMINATED` BY '\n';
```
...
load data local infile 'var/lib/mysql-files/load_user_100w_sort.sql' into table 'tb_user' fileds terminated by ',' lines terminated by '\n';

![keySort](D:\算法Typora\picture\keySort.png)

但是如果主键是乱序插入的话，就会导致需要插入的位置为中间的位置，会有页分裂的过程。

**页分裂：**页可以为空，也可以填充一般，也可以填充100%，每个页包含了2-N行数据（如果一行数据过大，会行溢出），根据主键排列。**主键是乱序插入时容易导致页分裂现象！**
...
- 满足业务需求的情况下，**尽量降低主键的长度**，**二级索引**的叶子节点保存的就是主键，所以主键小占用的空间也就会少。

- 插入数据时，尽量选择**顺序插入**，选择使用 AUTO_INCREMENT 自增主键

- 尽量不要使用 UUID 做主键或者是其他的自然主键，如身份证号，本身也是无序的，乱序插入会导致页分裂；占用的空间大。
...
![image-20220722140804693](D:\算法Typora\picture\image-20220722140804693.png)
...
本地的条件也会检查，还会向上检查。在向上找的时候，就要看是否上面开了检查选项，如果没开就不检查。和 CASCADED 的区别就是 CASCADED 不管上面开没开检查】选项都会进行检查。
...
存储过程思想上很简单，就是数据库SQL 语言层面的代码封装与重]用。
...
**【全局锁】**：`flash tables with read lock; mysqldump -uroot -p1234 表名>表名.sql(在命令行中sh)  unlock tables;`
...
2. 元数据锁（meta data lock，MDL），MDL加锁过程是系统自动控制，无需显式使用，在访问一张表的时候会自动加上。MDL锁主要作用是维护表元数据的数据一致性，在表上有活动事务的时候，不可以对元数据进行写入操作。在MySQL5.5中引入了MDL，当对一张表进行增删改查的时候，加MDL读锁（共享）;当对表结构进行变更操作的时候，加MDL写锁（排他）。  
   ...
   ![image-20220722210748929](D:\算法Typora\picture\image-20220722210748929.png)
...
​	InnoDB的数据是基于索引组织的，行锁是通过对索引上的索引项加锁来实现的，而不是对记录加的锁。对于**行级锁，主要分为以下三类：**  
...
2. InnoDB的行锁是针对于索引加的锁，**不通过索引条件检索数据**，那么InnoDB将对表中的所有记录加锁，此时**行锁就会升级为表锁。**
...
#### 📕：面试题：分析以下场景在可重复读隔离级别下会发生什么？

![image-20220804155743884](D:\算法Typora\picture\image-20220804155743884.png)

![image-20220804160250895](D:\算法Typora\picture\image-20220804160250895.png)
...
MySQL5.5版本开始，默认使用InnoDB存储引擎，它**擅长事务处理，具有崩溃恢复特性**，在日常开发中使用非常广泛。下面是InnoDB架构图，左侧为内存结构，右侧为磁盘结构。

![artic](D:\算法Typora\picture\artic.png)



Buffer Pool：缓冲池是主内存中的一个区域，里面可以缓存磁盘上经常操作的真实数据，在执行增删改查操作时，先操作缓冲池中的数据（若缓冲池没有数据，则从磁盘加载并缓存），然后再以一定频率刷新到磁盘，从而减少磁盘I0，加快处理速度。

![artic2](D:\算法Typora\picture\artic2.png)

![artic3](D:\算法Typora\picture\artic3.png)

![artic4](D:\算法Typora\picture\artic4.png)

![image-20220722224747798](D:\算法Typora\picture\image-20220722224747798.png)

磁盘架构：
![artic6](D:\算法Typora\picture\artic6.png)

![artic7](D:\算法Typora\picture\artic7.png)

![artic8](D:\算法Typora\picture\artic8.png)



InnoDB存储引擎涉及的**后台线程**：

![image-20220722225809265](D:\算法Typora\picture\image-20220722225809265.png)
...
![image-20220722231100776](D:\算法Typora\picture\image-20220722231100776.png)

> 个人理解： 事物每次提交的时候都会将数据刷到redo log中而不是直接将buffer pool中的数据直接刷到磁盘中（ibd文件中），是因为**redo log 是顺序写**，性能处理的够快，**直接从buffer刷到ibd中是随机写，性能慢**。所以脏页是在下一次读的时候，或者后台线程采用一定的机制进行刷盘到ibd中。
...
![image-20220722231940373](D:\算法Typora\picture\image-20220722231940373.png)

`ibd2sdi xxx.ibd`->查看表空间文件的基本信息
...
**总结：**不同事务或相同事务对同一条记录进行修改，会导致该记录的undolog生成一条记录版本链表，链表的头部是最新的旧记录，链表尾部是最早的旧记录。

![image-20220722232928504](D:\算法Typora\picture\image-20220722232928504.png)

如何确定返回哪一个版本 这是由read view决定返回 undo log 中的哪一个版本。

![image-20220722233210968](D:\算法Typora\picture\image-20220722233210968.png)

![image-20220722233347047](D:\算法Typora\picture\image-20220722233347047.png)
 ...
 **MVCC中在RC隔离级别下执行快照读时的流程：**![image-20220722234113430](D:\算法Typora\picture\image-20220722234113430.png)
