...
![](D:\算法Typora\picture\MySQL-体系结构.png)
...
<img src="D:\算法Typora\picture\MySQL-SQL的执行流程.png" style="zoom: 33%;" />
...
![](D:\算法Typora\picture\MySQL-权限范围.png)
...
![](D:\算法Typora\picture\MySQL-SHOW_PROCESSLIST命令.png)
...
4. 查看查询缓存的状态变量：
   ...
   <img src="D:\算法Typora\picture\MySQL-查询缓存的状态变量.png" style="zoom:67%;" />
...
* 备份
  ...
  ![图形化界面备份](D:\算法Typora\picture\图形化界面备份.png)

* 恢复

  ![图形化界面恢复](D:\算法Typora\picture\图形化界面恢复.png)
...
- SQL 分类
  ...
  - DCL（Data Control Language）数据控制语言
    ...
    ![](D:\算法Typora\picture\MySQL-SQL分类.png)
...
* 例如：
  ...
  <img src="D:\算法Typora\picture\MySQL-DQL数据准备.png" style="zoom: 80%;" />
...
* 例如
  ...
  ![](D:\算法Typora\picture\MySQL-DQL分页查询图解.png)
...
![](D:\算法Typora\picture\多表设计一对一.png)
...
![多表设计一对多](D:\算法Typora\picture\多表设计一对多.png)
...
![](D:\算法Typora\picture\多表设计多对多.png)
...
![](D:\算法Typora\picture\MySQL-JOIN查询图.png)
...
* 数据准备
  ...
  ![](D:\算法Typora\picture\自关联查询数据准备.png)
...
![多表练习架构设计](D:\算法Typora\picture\多表练习架构设计.png)
...
* 操作方式：
  ...
  * 对 MERGE 表进行 DROP 操作，但是这个操作只是删除 MERGE 表的定义，对内部的表是没有任何影响的
  ...
  ![](D:\算法Typora\picture\MySQL-MERGE.png)
...
![](D:\算法Typora\picture\MySQL-索引的介绍.png)
...
![](D:\算法Typora\picture\MySQL-组合索引图.png)
...
![](D:\算法Typora\picture\MySQL-InnoDB聚簇和辅助索引结构.png)
...
![](D:\算法Typora\picture\MySQL-聚簇索引和辅助索引检锁数据图.jpg)
...
![](D:\算法Typora\picture\MySQL-MyISAM主键和辅助索引结构.png)
...
* 插入前 4 个字母 C N G A 
  ...
  ![](D:\算法Typora\picture\MySQL-BTree工作流程1.png)

* 插入 H，n>4，中间元素 G 字母向上分裂到新的节点

  ![](D:\算法Typora\picture\MySQL-BTree工作流程2.png)

* 插入 E、K、Q 不需要分裂

  ![](D:\算法Typora\picture\MySQL-BTree工作流程3.png)

* 插入 M，中间元素 M 字母向上分裂到父节点 G

  ![](D:\算法Typora\picture\MySQL-BTree工作流程4.png)

* 插入 F，W，L，T 不需要分裂

  ![](D:\算法Typora\picture\MySQL-BTree工作流程5.png)

* 插入 Z，中间元素 T 向上分裂到父节点中

  ![](D:\算法Typora\picture\MySQL-BTree工作流程6.png)

* 插入 D，中间元素 D 向上分裂到父节点中，然后插入 P，R，X，Y 不需要分裂

  ![](D:\算法Typora\picture\MySQL-BTree工作流程7.png)

* 最后插入 S，NPQR 节点 n>5，中间节点 Q 向上分裂，但分裂后父节点 DGMT 的 n>5，中间节点 M 向上分裂

  ![](D:\算法Typora\picture\MySQL-BTree工作流程8.png)
...
<img src="D:\算法Typora\picture\MySQL-B加Tree数据结构.png" style="zoom:67%;" />
...
* 查看 SQL 执行频率：
  ...
  ![](D:\算法Typora\picture\MySQL-SQL语句执行频率.png)
...
* 查询 SQL 语句影响的行数：
  ...
  ![](D:\算法Typora\picture\MySQL-SQL语句影响的行数.png)
...
![](D:\算法Typora\picture\MySQL-explain查询SQL语句的执行计划.png)
...
![](D:\算法Typora\picture\MySQL-执行计划环境准备.png)
...
* id 相同时，执行顺序由上至下。连接查询的执行计划，记录的 id 值都是相同的，出现在前面的表为驱动表，后面为被驱动表
  ...
  ![](D:\算法Typora\picture\MySQL-explain之id相同.png)
...
* id 不同时，id 值越大优先级越高，越先被执行
  ...
  ![](D:\算法Typora\picture\MySQL-explain之id不同.png)
...
* id 有相同也有不同时，id 相同的可以认为是一组，从上往下顺序执行；在所有的组中，id 的值越大的组，优先级越高，越先执行
  ...
  ![](D:\算法Typora\picture\MySQL-explain之id相同和不同.png)
...
* 通过 have_profiling 参数，能够看到当前 MySQL 是否支持 profile：
  ![](D:\算法Typora\picture\MySQL-have_profiling.png)

* 默认 profiling 是关闭的，可以通过 set 语句在 Session 级别开启 profiling：

  ![](D:\算法Typora\picture\MySQL-profiling.png)
...
* 执行 SHOW PROFILES 指令， 来查看 SQL 语句执行的耗时:
  ...
  ![](D:\算法Typora\picture\MySQL-查看SQL语句执行耗时.png)
...
![](D:\算法Typora\picture\MySQL-优化SQL使用索引环境准备.png)
...
* 全值匹配：对索引中所有列都指定具体值，这种情况索引生效，执行效率高
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引1.png)
...
* **最左前缀法则**：联合索引遵守最左前缀法则
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引2.png)
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引3.png)
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引4.png)
...
* **范围查询**右边的列，不能使用索引：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引5.png)
...
* 在索引列上**函数或者运算（+ - 数值）操作**， 索引将失效：会破坏索引值的有序性
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引6.png)
...
* **字符串不加单引号**，造成索引失效：隐式类型转换，当字符串和数字比较时会**把字符串转化为数字**
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引7.png)
...
* **用 OR 分割条件，索引失效**，导致全表查询：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引10.png)
  ...
  **AND 分割的条件不影响**：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引11.png)
...
* **以 % 开头的 LIKE 模糊查询**，索引失效：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引12.png)
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引13.png)
...
* 如果 MySQL 评估使用索引比全表更慢，则不使用索引，索引失效：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引14.png)
...
* IS  NULL、IS NOT NULL  **有时**索引失效：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用索引15.png)
...
<img src="D:\算法Typora\picture\MySQL-索引失效底层原理1.png" style="zoom:67%;" />
...
* 范围查询右边的列，不能使用索引，比如语句： `WHERE a > 1 AND b = 1 `，在 a 大于 1 的时候，b 是无序的，a > 1 是扫描时有序的，但是找到以后进行寻找 b 时，索引树就不是有序的了
  ...
  <img src="D:\算法Typora\picture\MySQL-索引失效底层原理2.png" style="zoom:67%;" />

* 以 % 开头的 LIKE 模糊查询，索引失效，比如语句：`WHERE a LIKE '%d'`，前面的不确定，导致不符合最左匹配，直接去索引中搜索以 d 结尾的节点，所以没有顺序
      ![](D:\算法Typora\picture\MySQL-索引失效底层原理3.png)
...
![](D:\算法Typora\picture\MySQL-优化SQL查看索引使用情况.png)
...
![](D:\算法Typora\picture\MySQL-优化SQL使用索引8.png)
...
![](D:\算法Typora\picture\MySQL-优化SQL使用索引9.png)
...
![](D:\算法Typora\picture\MySQL-优化SQL load data.png)
...
1. **主键顺序插入**：因为 InnoDB 类型的表是按照主键的顺序保存的，所以将导入的数据按照主键的顺序排列，可以有效的提高导入数据的效率，如果 InnoDB 表没有主键，那么系统会自动默认创建一个内部列作为主键
   ...
   * 插入 ID 顺序排列数据：
   ...
   ![](D:\算法Typora\picture\MySQL-优化SQL插入ID顺序排列数据.png)

   * 插入 ID 无序排列数据：

   ![](D:\算法Typora\picture\MySQL-优化SQL插入ID无序排列数据.png)

2. **关闭唯一性校验**：在导入数据前执行 `SET UNIQUE_CHECKS=0`，关闭唯一性校验；导入结束后执行 `SET UNIQUE_CHECKS=1`，恢复唯一性校验，可以提高导入的效率。

   ![](D:\算法Typora\picture\MySQL-优化SQL插入数据关闭唯一性校验.png)
...
3. **手动提交事务**：如果应用使用自动提交的方式，建议在导入前执行`SET AUTOCOMMIT=0`，关闭自动提交；导入结束后再打开自动提交，可以提高导入的效率。
   ...
   ![](D:\算法Typora\picture\MySQL-优化SQL插入数据手动提交事务.png)
...
* 第一种是通过对返回数据进行排序，所有不通过索引直接返回结果的排序都叫 FileSort 排序，会在内存中重新排序
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL ORDER BY排序1.png)
...
* 第二种通过有序索引顺序扫描直接返回**有序数据**，这种情况为 Using index，不需要额外排序，操作效率高
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL ORDER BY排序2.png)
...
* 多字段排序：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL ORDER BY排序3.png)
...
* 分组查询：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL GROUP BY排序1.png)
...
* 查询包含 GROUP BY 但是用户想要避免排序结果的消耗， 则可以执行 ORDER BY NULL 禁止排序：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL GROUP BY排序2.png)
...
* 创建索引：索引本身有序，不需要临时表，也不需要再额外排序
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL GROUP BY排序3.png)
...
* 执行查询语句：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL OR条件查询1.png)
...
* 使用 UNION 替换 OR，求并集：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL OR条件查询2.png)
...
* 执行计划：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL嵌套查询1.png)
...
* 优化后：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL嵌套查询2.png)
...
* 分页查询：
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL分页查询1.png)
...
* 优化方式一：内连接查询，在索引列 id 上完成排序分页操作，最后根据主键关联回原表查询所需要的其他列内容
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL分页查询2.png)
...
* 优化方式二：方案适用于主键自增的表，可以把 LIMIT 查询转换成某个位置的查询
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL分页查询3.png)
...
* USE INDEX：在查询语句中表名的后面添加 USE INDEX 来提供 MySQL 去参考的索引列表，可以让 MySQL 不再考虑其他可用的索引
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用提示1.png)
...
* IGNORE INDEX：让 MySQL 忽略一个或者多个索引，则可以使用 IGNORE INDEX 作为提示
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用提示2.png)
...
* FORCE INDEX：强制 MySQL 使用一个特定的索引
  ...
  ![](D:\算法Typora\picture\MySQL-优化SQL使用提示3.png)
...
* 计数直接放到数据库里单独的一张计数表中，利用事务解决计数精确问题：
  ...
  <img src="D:\算法Typora\picture\MySQL-计数count优化.png" style="zoom: 50%;" />
...
<img src="D:\算法Typora\picture\MySQL-空闲链表.png" style="zoom: 50%;" />
...
<img src="D:\算法Typora\picture\MySQL-脏页链表.png" style="zoom:50%;" />
...
<img src="D:\算法Typora\picture\MySQL-查询内存优化.png" style="zoom:50%;" />
...
<img src="D:\算法Typora\picture\MySQL-删除数据.png" style="zoom:50%;" />
...
<img src="D:\算法Typora\picture\MySQL-重建表.png" style="zoom: 67%;" />
...
![](D:\算法Typora\picture\MySQL-MVCC版本链隐藏字段.png)
...
<img src="D:\算法Typora\picture\MySQL-MVCC版本链.png" style="zoom: 80%;" />
...
![](D:\算法Typora\picture\MySQL-MVCC工作流程1.png)
...
![](D:\算法Typora\picture\MySQL-MVCC工作流程2.png)
...
<img src="D:\算法Typora\picture\MySQL-update的执行流程.png" style="zoom: 33%;" />
...
![](D:\算法Typora\picture\MySQL-MyISAM 锁的兼容性.png)
...
* C1、C2 加读锁，同时查询可以正常查询出数据
  ...
  ![](D:\算法Typora\picture\MySQL-MyISAM 读锁1.png)
...
* C1 加读锁，C1、C2 查询未锁定的表，C1 报错，C2 正常查询
  ...
  ![](D:\算法Typora\picture\MySQL-MyISAM 读锁2.png)
  ...
  ![](D:\算法Typora\picture\MySQL-MyISAM 读锁3.png)
...
* C1 加写锁，C1、C2查询表，C1 正常查询，C2 需要等待
  ...
  ![](D:\算法Typora\picture\MySQL-MyISAM 写锁1.png)
...
* C1、C2 同时加写锁
  ...
  ![](D:\算法Typora\picture\MySQL-MyISAM 写锁2.png)
...
* 查看锁竞争：
  ...
  ![](D:\算法Typora\picture\MySQL-锁争用情况查看1.png)
  ...
  ![](D:\算法Typora\picture\MySQL-锁争用情况查看2.png)
...
* 查看锁状态：
  ...
  ![](D:\算法Typora\picture\MySQL-MyISAM 锁状态.png)
...
* 查询 id 为 3 的数据，正常查询：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁操作1.png)
...
* C1 更新 id 为 3 的数据，但不提交：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁操作2.png)
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁操作3.png)
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁操作4.png)
...
* C1 更新 id 为 3 的数据，但不提交，C2 也更新 id 为 3 的数据：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁操作5.png)
...
* 操作不同行的数据：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁操作6.png)
...
* 查询数据表：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 间隙锁1.png)
...
* C1 根据 id 范围更新数据，C2 插入数据：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 间隙锁2.png)
...
![](D:\算法Typora\picture\MySQL-意向锁兼容性.png)
...
* 执行更新语句：
  ...
  ![](D:\算法Typora\picture\MySQL-InnoDB 锁升级.png)
...
<img src="D:\算法Typora\picture\MySQL-InnoDB 锁争用.png" style="zoom: 80%;" />
...
![](D:\算法Typora\picture\MySQL-InnoDB查看锁状态.png)
...
![](D:\算法Typora\picture\MySQL-主从复制原理图.jpg)
...
* 分流查询：通过 MySQL 的主从复制，实现读写分离，使增删改操作走主节点，查询操作走从节点，从而可以降低单台服务器的读写压力
  ...
  ![](D:\算法Typora\picture\MySQL-负载均衡主从复制.jpg)
...
4. 查看 master 状态：
   ...
   ![](D:\算法Typora\picture\MySQL-查看master状态.jpg)
...
2. 在从库中查询数据，进行验证：
   ...
   ![](D:\算法Typora\picture\MySQL-主从复制验证1.jpg)

   在该数据库中，查询表中的数据：

   ![](D:\算法Typora\picture\MySQL-主从复制验证2.jpg)
...
* 查看日志内容：
  ...
  ![](D:\算法Typora\picture\MySQL-日志读取1.png)
...
* 查看日志内容：日志格式 ROW，直接查看数据是乱码，可以在 mysqlbinlog 后面加上参数 -vv 
  ...
  ![](D:\算法Typora\picture\MySQL-日志读取2.png)
...
![](D:\算法Typora\picture\MySQL-查询日志.png)
...
* 直接通过 cat 指令查询该日志文件：
  ...
  ![](D:\算法Typora\picture\MySQL-慢日志读取1.png)
...
* 如果慢查询日志内容很多，直接查看文件比较繁琐，可以借助 mysql 自带的 mysqldumpslow 工具对慢查询日志进行分类汇总：
  ...
  ![](D:\算法Typora\picture\MySQL-慢日志读取2.png)
...
![](D:\算法Typora\picture\普通表.png)
					

第一范式表：

![](D:\算法Typora\picture\第一范式.png)
